...
for i in range(0,2000,200):
    ...
    coordinates=[]
    def YZ(N,R,label,M,Q):
        initial=eF
        final=initial+200
        for num in range(initial,final,200):
            data1 = np.loadtxt('YZ_piezo_{}.dat'.format(num))
            data2 = np.loadtxt('YZ_{}.dat'.format(num))
    ...
    for i in range(2,33):
        YZ(N,3,'Planexy_{}'.format(i),i,'e11')
        
    savetxt('yz{}({}) {}kV.dat'.format(N,fO,eF), coordinates, fmt='%4d')
    np.save("yz{}({}) {}kV".format(N,fO,eF), coordinates)